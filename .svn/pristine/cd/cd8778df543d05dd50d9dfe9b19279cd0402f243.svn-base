			...
			<view id="content-view" :style="'height:'+contentHeight+'px;'">
				...
				<template v-if="!userInfo.uid">
					<view class="li-view">
						...
						<!-- #ifdef APP-PLUS || H5 -->
						<vip-info :level="level" :member_validate_dates="appDates" @memberLogin="noLogin" :platform="platform" programNumber="3"></vip-info>
						<!-- #endif -->
				...
				<template v-else>
					<view class="li-view">
						<view id="header-member-info">
							<view class="zx">
								...
								<button  class="login-btn-avatar" @tap="editProfile" v-if="!userInfo.avatarUrl">
									<image class="tx-img-nologin" src="../../static/img/user/people.png"></image>
								</button>
						...
						<view id="header-view" @tap="logout">
							<view class="name">
								<text class="nickname" v-if="userInfo.nickName != undefined">
									{{userInfo.nickName}}
								</text>
								<view class="navigator-css">
				...
				<!-- #ifdef MP-WEIXIN || MP-QQ -->
				<view id="bottom" :style="'height:'+bottomHeight+'px;'">
					<view id="bottom_list">
						<uni-list>
				...
				<!-- #ifdef APP-PLUS || H5 --> 
				<view id="app-bottom" :style="'height:'+(bottomHeight)+'px;'">
					<view id="app-bottom_list">
						<uni-list style="margin-top:140rpx;width:686rpx;position: absolute;">
							...
							<!-- #endif -->
							<uni-list-item  thumb="/static/img/user/contact_customer.png"  title="联系客服" @tap="copy_customer_wechat()" :showArrow="false">
								<template v-slot:right="">
									<view style="display: flex;align-items: center;" @tap="copy_customer_wechat">
							...
							<uni-list-item title="当前版本" thumb="/static/img/user/cur_version.png" :showArrow="false">
								<template v-slot:right="">
									<text class="bottom-version-text">{{ version }}</text>
								</template>
							</uni-list-item>
						</uni-list>
					</view>
...
<script>
	...
	export default {
		...
		async onLoad(option) {
			...
			this.bottomHeight = winHeight -50- uni.upx2px(330);
		...
		computed: {
			...
			version: function() {
				...
				const androidVer = d.version_android ? d.version_android:'';
				const iosVer = d.version_mac ? d.version_mac:'';
				return this.platform == 2 ? iosVer:androidVer;
			},
			wx: function() {
		...
		methods: {
			...
			async getConfig() {
				...
				console.log('config resp:', respData);
