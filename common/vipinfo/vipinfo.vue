	...
	<view class="header-vip-view" :style="'background-image:'+backgroundImage" @tap="payForVip()">
		<template v-if="level<=0">
			<button type='primary' @tap="appLogin" open-type="getUserInfo" withCredentials="true" lang="zh_CN" @getuserinfo="wxGetUserInfo" class="login-btn-view">
				...
				<view class="right-vip-image-nologin-view" v-if="programNumber!=3">
		...
		computed:{
			memberAction: function() {
				...
				if(this.level == 0) {
					...
					return this.platform == 1? '续费':'';
				}
			},
			memberDiscountText: function() {
				console.log('this.platform', this.platform, 'programNumber', this.programNumber);
				// 不为APP 时候
				if (this.programNumber !=3) {
			...
			backgroundImage: function() {
				...
				if(this.platform == 2 && this.programNumber == 3) {
					url = 'https://imgmyqx.ofbei.com/images/ios_no_member_left_vip_logo.png';
				}
				console.log('url', url);
				let s =  "url('"+url+"');";
				return s;
